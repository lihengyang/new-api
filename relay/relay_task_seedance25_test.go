...
func TestRelayTaskSubmitRejectsSeedance25InvalidRequestsBeforeBilling(t *testing.T) {
	tests := []struct {
		...
		{name: "missing duration", body: `{"prompt":"p","model":"seedance-2.5","metadata":{"resolution":"720p"}}`},
		{name: "unsupported resolution", body: `{"prompt":"p","model":"seedance-2.5","metadata":{"duration":4,"resolution":"1080p"}}`},
		{name: "disabled false field", body: `{"prompt":"p","model":"seedance-2.5","metadata":{"duration":4,"resolution":"720p","camera_fixed":false}}`},
		{name: "reference audio", body: `{"prompt":"p","model":"seedance-2.5","metadata":{"duration":4,"resolution":"720p","content":[{"type":"audio_url","role":"reference_audio","audio_url":{"url":"https://example.invalid/a.wav"}}]}}`},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c, info := newTaskClientRequestIDContext(t, tt.body)
			info.OriginModelName = relaycommon.Seedance25PublicAlias
			c.Set("model_mapping", `{"seedance-2.5":"mapped-provider-model-placeholder"}`)
...
func TestRelayTaskSubmitRejectsSeedance25MissingMappingBeforeBilling(t *testing.T) {
	body := `{"prompt":"p","model":"seedance-2.5","metadata":{"duration":4,"resolution":"720p"}}`
...
func TestSeedance25VideoFetchDoesNotExposePrivateRoutingOrBillingContext(t *testing.T) {
	...
	require.Contains(t, string(body), relaycommon.Seedance25PublicAlias)
