...
func TestRelayTaskSubmitRejectsMappedMiniHighResolutionBeforeBilling(t *testing.T) {
	for _, resolution := range []string{"1080p", "4k"} {
		t.Run(resolution, func(t *testing.T) {
			body := fmt.Sprintf(
				`{"prompt":"hello","model":"lsf-seedance-2.0-mini-henrytest","metadata":{"resolution":%q}}`,
				resolution,
			)
			c, info := newTaskClientRequestIDContext(t, body)
			info.OriginModelName = "lsf-seedance-2.0-mini-henrytest"
			c.Set("model_mapping", `{"lsf-seedance-2.0-mini-henrytest":"dreamina-seedance-2-0-mini-260615"}`)
